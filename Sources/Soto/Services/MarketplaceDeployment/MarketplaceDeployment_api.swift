...
public struct MarketplaceDeployment: AWSService {
    ...
    public func putDeploymentParameter(_ input: PutDeploymentParameterRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<PutDeploymentParameterResponse> {
        return self.client.execute(operation: "PutDeploymentParameter", path: "/catalogs/{catalog}/products/{productId}/deployment-parameters", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
