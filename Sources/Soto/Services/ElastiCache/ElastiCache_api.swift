...
public struct ElastiCache: AWSService {
    ...
    public func testMigration(_ input: TestMigrationMessage, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<TestMigrationResponse> {
        return self.client.execute(operation: "TestMigration", path: "/", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
}
