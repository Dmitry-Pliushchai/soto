...
extension PersonalizeRuntime {
    ...
    public func getActionRecommendations(_ input: GetActionRecommendationsRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> GetActionRecommendationsResponse {
        return try await self.client.execute(operation: "GetActionRecommendations", path: "/action-recommendations", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
