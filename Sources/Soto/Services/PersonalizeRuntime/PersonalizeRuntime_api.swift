...
public struct PersonalizeRuntime: AWSService {
    ...
    public func getActionRecommendations(_ input: GetActionRecommendationsRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<GetActionRecommendationsResponse> {
        return self.client.execute(operation: "GetActionRecommendations", path: "/action-recommendations", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
