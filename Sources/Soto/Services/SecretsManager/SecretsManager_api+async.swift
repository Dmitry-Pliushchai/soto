...
extension SecretsManager {
    ...
    public func batchGetSecretValue(_ input: BatchGetSecretValueRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> BatchGetSecretValueResponse {
        return try await self.client.execute(operation: "BatchGetSecretValue", path: "/", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
