...
public struct EKSAuth: AWSService {
    ...
    public func assumeRoleForPodIdentity(_ input: AssumeRoleForPodIdentityRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<AssumeRoleForPodIdentityResponse> {
        return self.client.execute(operation: "AssumeRoleForPodIdentity", path: "/clusters/{clusterName}/assume-role-for-pod-identity", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
}
