...
extension VerifiedPermissions {
    ...
    public func batchIsAuthorized(_ input: BatchIsAuthorizedInput, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) async throws -> BatchIsAuthorizedOutput {
        return try await self.client.execute(operation: "BatchIsAuthorized", path: "/", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
