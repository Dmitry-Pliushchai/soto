...
public struct SSOOIDC: AWSService {
    ...
    public func createTokenWithIAM(_ input: CreateTokenWithIAMRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<CreateTokenWithIAMResponse> {
        return self.client.execute(operation: "CreateTokenWithIAM", path: "/token?aws_iam=t", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
