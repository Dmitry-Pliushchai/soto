...
public struct MigrationHubConfig: AWSService {
    ...
    public func deleteHomeRegionControl(_ input: DeleteHomeRegionControlRequest, logger: Logger = AWSClient.loggingDisabled, on eventLoop: EventLoop? = nil) -> EventLoopFuture<DeleteHomeRegionControlResult> {
        return self.client.execute(operation: "DeleteHomeRegionControl", path: "/", httpMethod: .POST, serviceConfig: self.config, input: input, logger: logger, on: eventLoop)
    }
